...
Встроенная сборка — родная утёкшая pre-release **310.8.0** NVIDIA (из
зеркала RankFTW/rhi-repo): разбор fatbin-заголовков показывает только
ядра `sm_120` (Blackwell). На RTX 40 хук архитектуры (ниже) заставляет
feature создаваться — подтверждено пользователем на 4080 Super. Отказ на
старых картах — проверка политики, а не отсутствие кода.
