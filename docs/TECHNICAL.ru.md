...
Оригинальная сборка NVIDIA несёт ядра только для `sm_120` (Blackwell).
Встроенная сборка — community-пересборка **310.8.0-RTX40** (из зеркала
RankFTW/rhi-repo): разбор fatbin-заголовков показывает `sm_89` (Ada) и
`sm_120` (Blackwell) — то есть работает на RTX 40 и RTX 50. Отказ на
