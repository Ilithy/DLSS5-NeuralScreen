...
- **Видеокарта NVIDIA RTX.** Честно, что реально работает:
  ...
  | **RTX 40** (Ada) | ✅ работает - встроенный рантайм несёт ядра sm_89 |
...
- **Встроенный `nvngx_dlssnr.dll` — community-пересборка** утёкшего pre-release рантайма NVIDIA 310.8.0 (ядра sm_89 + sm_120: RTX 40 и RTX 50) — см. Лицензию ниже.
...
Код здесь под MIT. `nvngx_dlssnr.dll` — community-пересборка утёкшего pre-release рантайма NVIDIA 310.8.0 (оригинал несёт ядра sm_120 только для RTX 50; эта сборка добавляет sm_89 для RTX 40). Включена как есть, нами не изменялась, без гарантий; относитесь к ней как к research-only ПО.
