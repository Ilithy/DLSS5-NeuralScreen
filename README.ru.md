...
Запуск и переключения режимов не мигают: окна оверлея создаются скрытыми и
появляются вместе с первым настоящим кадром, а при смене режима (Num5, смена
монитора) на время перестройки конвейера показывается лёгкая полупрозрачная
«заглушка» с блуром и спиннером — картинка возвращается резкой.
...
В **режиме одного окна** (Num5) входом становится одно окно вместо рабочего
стола, поэтому петли самозахвата нет: оверлей перестаёт прятаться от захвата,
и внешние рекордеры (захват экрана OBS, NVIDIA App) видят обработанную
картинку. В полноэкранном режиме скрытие остаётся.
...
положение. (С residual composite картинка остаётся резкой вплоть до
пониженных разрешений; мылится только самый низ ползунка.)
...
- **HDR-дисплеи не поддерживаются**: весь конвейер работает в SDR (sRGB).
  На HDR-экране картинка выходит слишком яркой и выцветшей — переключите
  дисплей в SDR (Win+Alt+B), пока программа работает.
- **Задержка конвейера 40–60 мс** (несколько кадров на обычной частоте
  рабочего стола) — сам нейропроход занимает доли миллисекунды на
  мегапиксель, но цепочка захват → обработка → вывод накапливает
  задержку. Для интерактивной работы нормально, для соревновательной
  игры нет.
- **Разрешение обработки ограничено 2560×1440** по широкой стороне —
  сама нейросеть отказывается работать в 4K (проверено, виснет). Вывод
  всегда в вашем родном разрешении; уменьшается только внутренний рабочий
  размер сети.
...
Код здесь под MIT. `nvngx_dlssnr.dll` — не мой файл, но он входит в архив
релиза — это файл NVIDIA, и он на её условиях.
