...
оверлеем, прозрачным для кликов. Официально — RTX 50-серии; для
20/30/40-серий см. «Старые GPU» ниже.
...
> RTX 5070 Ti, рабочий стол 2560×1600, work на потолке 2304×1440:
> **102 FPS с NR**. Из 9.8 мс кадра 8.0 мс — GPU-время внутри NGX evaluate
> (замерено таймстемпами D3D12 на очереди), вся остальная обвязка — 1.8 мс.
...
тянуло за собой весь tcl/tk в runtime. В меню: живые показания (FPS,
разрешение, work-размер, кадры, время записи), NR вкл/выкл, профиль
(Faithful / Natural / Strong / Extreme), четыре параметра NR, шторка
«до/после», язык (ru/en), светлая/тёмная тема, «открывать меню при запуске»
и кнопки Скриншот / Справка / Запись / Выход / Закрыть. Строка статуса
показывает GPU, его архитектуру и работает ли на нём Neural Rendering.

Панель таскается за заголовок и растягивается за правый нижний угол — обе
зоны подсвечиваются при наведении, и обе запоминаются в `config.json`
(`menu_offset`, `menu_scale`).
...
| | Что делает |
|---|---|
| `Ctrl+Alt+Q`, «Выход» в меню, «Выход» в трее | завершают программу: воркер погашен, окна закрыты, процессов не остаётся |
| «Закрыть» в меню, `F8`, `Esc` | только прячут меню, программа продолжает работать |
...
`Evaluate`, а не временем вокруг submit: **7.9–8.6 мс без изменений от 0.37 до
3.32 МПикс** (рабочий стол 2560×1600, RTX 5070 Ti). Пикселей на входе в девять
раз больше, время то же — модель считает на своём фиксированном внутреннем
разрешении. Сквозной FPS одинаков на `0.65` и `1.00`: 102. Это регулятор
...
`NvAPI_GPU_GetArchInfo` по id. При `NS_ARCH_SPOOF=1` воркер патчит эту одну
функцию **в памяти своего процесса** при старте: пролог сохраняется,
заменяется прыжком на наш обработчик и восстанавливается вокруг каждого
реального вызова — любой хендл GPU по-прежнему обслуживает родной код
NVIDIA, переписывается только возвращаемая архитектура (на Blackwell).
Файлы NVIDIA не модифицируются, а на 50-серии хук сам себя отключает и
ничего не делает.

Выключено по умолчанию. **Не проверено на реальном железе 20/30/40-серий** —
здесь только 5070 Ti, где хук по построению no-op. Точка GPU в меню говорит
правду в любом случае: она зеленеет только когда воркер реально создал
feature 18, а не когда архитектура просто выглядит правильно.
