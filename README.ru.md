...
| `Insert` | старт/стоп записи (MP4: AV1 NVENC 60 кадров/с ~64 Мбит/с + системный звук AAC). Есть и кнопка **Запись** в меню |
...
- **скорость** — обработка в пониженном разрешении и само разрешение.
  По умолчанию выключено: быстрее, мягче — см. Производительность
...
| `nr_small` | обрабатывать в пониженном разрешении и растягивать результат: быстрее, мягче. По умолчанию `false` |
...
**По умолчанию выключено**, и намеренно: это на 52% быстрее и заметно мягче,
потому что обычный билинейный апскейл не возвращает ту детализацию, которую
сеть только что добавила. В играх аддону выше этот трюк сходит с рук, потому
что растягиванием занимается собственный DLSS Super Resolution игры; на
рабочем столе такого нет. Включите, посмотрите на свой экран, решите сами.
...
2. **Внешние рекордеры (OBS, ShadowPlay, Lightshot) НЕ захватывают оверлей.**
   ...
   реальный NR-кадр вместе с системным звуком, и с меню, если оно открыто.
