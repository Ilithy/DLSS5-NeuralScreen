...
- **Видеокарта NVIDIA RTX.** Честно, что реально работает:
  ...
  | **Ноутбуки с гибридной графикой (Optimus)** | ⚠️ работает только когда дисплей ведёт дискретная GPU - принудительно dGPU (MUX-переключатель или внешний монитор на порт dGPU). На встроенной графике падает на первом кадре |
...
работает; картинка не обрабатывается. Обходного пути нет.
...
(игра, браузер)? Наведите мышь на него и нажмите **Num5** - или выберите
**«Выбрать окно...»** в меню. Оверлей следует за окном; вернуться на весь
экран - ещё раз **Num5** или **«Во весь экран»** в меню.
...
NVIDIA App его не видят. В режиме окна - видят. Запись: наведите на окно,
**Num5**, запустите запись в OBS / NVIDIA App, **Num5** когда закончите.
...
- **OBS (рекомендуется):** запустите с `NS_SPOUT=1` (переменная окружения,
  например `set NS_SPOUT=1` и `NeuralScreen.exe`). Воркер публикует вывод
  как Spout2 shared texture - добавьте в OBS источник **Spout2 Capture**
  (бесплатный плагин) и записывайте. Работает и в полноэкранном режиме.
  По умолчанию выключено.
- **NVIDIA App / OBS display capture:** используйте режим одного окна -
  наведите на окно, **Num5**, записывайте, **Num5** когда закончите. В этом
  режиме оверлей виден захвату экрана; в полноэкранном - прячется
...
(ядра sm_75/86/89/120, RTX 20-50). Включён как есть, без гарантий;
research-only.
