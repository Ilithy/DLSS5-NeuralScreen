...
Хоткеи настраиваются: в `config.json` добавьте `"hotkeys": {"toggle": "F9", "record": "Insert", ...}` (команды: `toggle`, `settings`, `record`, `scale_up`, `scale_down`, `quit`; клавиши: F1–F12, буквы, цифры, Insert/Delete/Home/End/PgUp/PgDn/стрелки, модификаторы Ctrl/Alt/Shift).
...
выбор монитора, автозапуск с Windows и кнопки Скриншот / Справка / Запись /
Выход / Закрыть. Строка статуса показывает GPU, его архитектуру и работает
ли на нём Neural Rendering. Скриншот открывает нативный диалог «Сохранить
как» (JPEG 100%).
