...
- **Видеокарта NVIDIA RTX.** Официально поддерживается 50-я серия. 20-я, 30-я и
  ...
  **Скорость:** на 20/30-й сериях нейропроход медленный — единицы или до ~20 FPS в 1440p, не 60+ как на 50-й. Эффект настоящий, частота — нет. Снизьте ползунок «Разрешение работы сети» — это главный рычаг скорости.
...
положение; мылится только самый низ ползунка.
...
секунды на кадр; хоткеи обрабатываются между кадрами — нажатие может
показаться потерянным. Картинка важнее.

## Известные ограничения

- **Настоящий полноэкранный режим игр** оверлеем не перекрыть — это правило
  Windows. Только borderless или окно.
- **Вторая копия не защищена** — две копии дерутся за захват; закройте
  первую перед запуском второй.
- **Список окон в меню** показывает все видимые окна; выбор переключает на
  него. Num5 по-прежнему берёт окно под курсором.
- **Второй монитор** работает, но окно между мониторами не тестировалось.
- **Позиция меню в режиме окна** при каждом открытии — правый нижний угол;
  перетащите куда нужно — запомнится при закрытии.
- **HDR-дисплеи не поддерживаются** — включите SDR (Win+Alt+B).
- **Ноутбуки с гибридной графикой** (Optimus: дисплей рисует встроенная
  видеокарта) не поддерживаются — оценка падает на первом кадре.
  Принудительно включите дискретную GPU, MUX-переключатель или внешний
  монитор к порту дискретной.
- **Задержка конвейера 40–60 мс** — интерактивно нормально, соревновательно
  нет.
- **Разрешение обработки ограничено 2560×1440** (сеть отказывается работать
  в 4K); вывод всегда в вашем родном разрешении.
