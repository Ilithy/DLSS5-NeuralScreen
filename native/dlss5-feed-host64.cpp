...
//   1 = frame payload read into color/mv, 2 = resize command read into rc, 0 = EOF/error.
static int ReadVideoMessage(VideoState &v, VideoFrameHeader &fh, std::vector<BYTE> &color,
                            std::vector<BYTE> &mv, VideoResizeCmd &rc)
{
    ...
    if (fh.magic == FRAME_MAGIC)
    {
        ...
        const size_t pixels = cw * ch;
        color.resize(pixels * 4); mv.resize(static_cast<size_t>(v.w) * v.hgt * 4);
...
static int RunVideo()
{
    ...
    {
        VideoResizeCmd rc = {};
        const int msg = ReadVideoMessage(v, fh, color, mv, rc);
        if (msg == 0)
        {
            if (live)
            {
